...
Following geophysical causality, the dataset is partitioned chronologically: in the six-year multi-annual experiment (2015–2020), the initial 54 months (January 2015 to June 2019, 75.0%) serve as the training set for backpropagation and parameter optimization; the subsequent 10 months (July 2019 to April 2020, 13.9%) serve as the validation set for hyperparameter tuning and early stopping; and the final 8 months (May 2020 to December 2020, 11.1%) serve as an independent test set for unobserved temporal extrapolation evaluation. The pipeline also fully supports annual benchmarks or user-specified year ranges via `--years`.
